...
def registration_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and formset.is_valid():
            ...
            return redirect('participant_profile')  # Возврат к списку анкет
    else:
...
def registration_edit(request, registration_id):
    ...
    registration = get_object_or_404(Registration, id=registration_id, participant=participant)

    if request.method == 'POST':
        form = RegistrationForm(request.POST, instance=registration)
        formset = ChildFormSet(request.POST, instance=registration)
        ...
        formset = ChildFormSet(instance=registration)
